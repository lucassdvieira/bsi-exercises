...
def nomes_com_menos_de_4_letras(lista):
    ...
    return [name for name in lista if len(name) <= 4]
...
def lista_de_nomes_e_inicial(lista):
    ...
    return [(name[0], name) for name in lista]
...
def mulher_homem(mulher, homem):
    ...
    return {m: h for m, h in zip(mulher, homem)}
...
def pares_e_divisiveis_por_7(limite_inicial=1067, limite_final=3627):
    ...
    return len(
        [
            item
            for item in range(limite_inicial, limite_final)
...
def conta_pares(nums):
    ...
    return len([odd for odd in nums if odd % 2 == 0])
...
def gago(texto):
    ...
    return "".join([newTxt[0] + "-" + newTxt + " " for newTxt in texto.split()])[:-1]
...
def explode_string(s):
    ...
    """
    return "".join([s[: index + 1] for index, _ in enumerate(s)])
...
def intercalamento_listas(lista1, lista2):
    ...
    zipList = [[x, y] for x, y in zip(lista1, lista2)]
    newList, c = [], 0
    while c < len(zipList):
        newList.append(zipList[c][0])
        newList.append(zipList[c][1])
        c += 1
    return newList
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    def testa_sorte(number):
        result, numberList = False, list(str(number))
        for i in numberList:
            if i == "2":
                result = True
            elif i == "7":
    ...
    return len([x for x in range(limite_inferior, limite_superior) if testa_sorte(x)])
