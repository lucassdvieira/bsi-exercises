...
def troca_primeira_e_ultima(texto):
    ...
    return texto[-1] + texto[1:-1] + texto[0] if len(texto) > 1 else texto
...
def nove_na_frente(lista):
    ...
    #     if i <= 3:
    #         lista_quatro.append(valor)

    # for i in lista_quatro:
    #     if i != 9:
    #         continue
    #     else:
    #         return True
    ...
    return True if 9 in lista[:4] else False
...
def metade_inicial(texto):
    ...
    return (texto[:(len(texto)//2)]) if len(texto) >= 2 else ''
...
def pontas_removidas(texto):
    ...
    return texto[1:-2] + texto[-2] if len(texto) > 2 else ''
...
def dois_giros_a_esquerda(texto):
    ...
    return texto[2:] + texto[:2] if len(texto) >= 1 else ''
...
def pontas_do_nome(nome):
    ...
    """
    return (nome.split(' ')[0] + ' ' + nome.split(' ')[-1]).upper()
