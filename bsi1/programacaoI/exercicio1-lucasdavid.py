...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    return math.floor(((anos_fumando * 365) * (cigarros_fumados_por_dia * 10)) / 1440)
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    return (((p1 * 7 / 10) + (ep1 * 3 / 10) + (p2 * 7 / 10) + (ep2 * 3 / 10)) / 2) >= 7
...
def salario(valor_hora, horas_mensais):
    ...
    desconto = ((valor_hora * horas_mensais) * 0.08) + ((valor_hora * horas_mensais) * 0.11) + ((valor_hora * horas_mensais) * 0.05)
    return round((valor_hora * horas_mensais) - desconto)
