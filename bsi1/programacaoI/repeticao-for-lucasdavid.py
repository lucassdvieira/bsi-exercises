...
def soma_das_temperaturas_dos_inteiros(valor1, valor2):
    ...
    lista = []
    if valor1 < valor2:
        primeiro, segundo = valor1, valor2
    else:    
        primeiro, segundo = valor2, valor1
...
def dar_troco(valor_a_pagar, valor_em_dinheiro):
    ...
    notas, troco, lista = [50, 20, 10, 5, 2, 1],  valor_em_dinheiro - valor_a_pagar, []
    for i in notas:
        if troco // i > 0:
            lista.append((i, (troco // i)))
            troco = troco - i * (troco // i)
    return lista
...
def media_anual(temperaturas):
    ...
    temperatura_media_anual, indice = sum(temperaturas) / len(temperaturas), []
    for i, valor in enumerate(temperaturas):
        if valor > temperatura_media_anual:
            indice.append(i)
    return indice
...
def maiores_13(idades, alturas):
    ...
    media_altura, altura_final = round(sum(alturas) / len(alturas), 2), []
    for a, b in zip(idades, alturas):
        if a > 13 and b < media_altura:
            altura_final.append(b)
    return altura_final
