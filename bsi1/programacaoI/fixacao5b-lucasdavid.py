...
def numero_invertido(numero):
    ...
    return int(''.join(list(str(numero)[::-1])))
...
def rosquinhas(n):
    ...
    """
    return 'muitas rosquinhas' if n >= 10 else str(n) + ' rosquinhas'
...
def nomes_pontas(n):
    ...
    return (n.split(' ')[0] + ' ' + n.split(' ')[-1]).upper()
...
def nomes_pontas_e_iniciais_do_meio(n):
    ...
    return (n[0] + ' ' + middleName + ' ' + n[-1] if len(middleName) > 0 else n[0] + ' ' +n[-1]).upper()
...
def tres_maiusculas(texto):
    ...
    for i, valor in enumerate(texto):
        if i != (len(texto) -1) and (valor == valor.upper()):
            if valor == texto[i+1] and valor == texto[i+2] :
                return i
