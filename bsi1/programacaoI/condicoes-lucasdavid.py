...
def acrescimo_nota_bb(nota_sozinho, nota_com_ajuda):
    ...
    """
    return 0.25 * (nota_com_ajuda - nota_sozinho)
...
def maior_dia_do_mes(mes, ano):
    ...
    """
    diamax, mesex = 30, [1, 3, 5, 7, 8, 10, 12]
    if mes == 2:
        if ano_bissexto(ano):
            diamax = 29
        else:
...
def data_valida(data):
    ...
    return False if dia > maior_dia_do_mes(mes,ano) or mes > 12 or mes < 1 or ano <= 0 else True 
...
def acrescimo_nota_bb(nota_sozinho, nota_com_ajuda):
    ...
    return round((nota_com_ajuda - nota_sozinho) * (1/4), 1) if (nota_com_ajuda - nota_sozinho) * (1/4) > 0 else 0
