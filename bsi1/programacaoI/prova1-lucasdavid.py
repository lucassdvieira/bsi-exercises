...
def aluguel_hotel_urbano(valor_diaria, dias):
    ...
    return (valor_diaria * dias) + ((valor_diaria * dias) * 0.06) + 65
...
def acesso_4G(qt_alunos_total, qt_alunos_4G):
    ...
    return round((qt_alunos_4G * 100) / qt_alunos_total, 2)
...
def media_ponderada(prova, trabalho, exercicio):
    ...
    return round(((prova * 5) + (trabalho * 3) + (exercicio * 1)) / (5 + 3 + 1),1)
...
def lan_house(valor_15_minutos, qt_minutos_uso):
    ...
    return math.ceil(qt_minutos_uso / 15) * valor_15_minutos
