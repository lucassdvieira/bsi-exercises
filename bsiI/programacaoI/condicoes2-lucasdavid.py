...
def situacao_aluno(nota1, nota2, nota3, faltas, aulas_ministradas):
    ...
    """
    faltas_porcentagem = aulas_ministradas * 0.25
    if faltas > faltas_porcentagem:
        return 'RF'
...
def aumento_preco(preco):
    ...
    if preco <= 200:
        ...
        porcentagem = 0.05 
    return (preco, (100 * porcentagem), ((preco * porcentagem)), (preco + (preco * porcentagem)))
...
def aumento_salario(salario):
    ...
    if salario <= sm:
        ...
        porcentagem = 0.05 
    return round((salario + (salario * porcentagem)), 2)
...
def converte_hora_24_para_12(horario):
    ...
    horas, minutos = horario[:2], horario[3:5]
    if horas[1] == ":":
        horas = "0" + horario[0]
        horario = "0" + horario[0:]
        minutos = horario[3:5]
...
def converte_hora_12_para_24(horario):
    ...
    am_pm = horario[5:8]
    am_pm = am_pm.replace(" ", "")
    if horas == "12" and am_pm == "am":
        return "00:" + minutos
    elif int(horas) <= 12 and am_pm == "am":
        return horas + ':' + minutos
    elif horas == "12" and am_pm == "pm":
        return horas + ":" + minutos
    elif int(horas) <= 12 and am_pm == "pm":
        horas = int(horas)
        horas = int(horas) + 12
        horas = str(horas)
        return horas + ":" + minutos
...
def conta_combustivel(qtde_litros, tipo_combustivel, tipo_pagamento):
    ...
    """
    alcool, gasolina, diesel = 3.159, 3.739, 3.090
    if tipo_combustivel == "a":
        valor = qtde_litros * alcool
        if tipo_pagamento == "v" or tipo_pagamento == "d":
            valor = valor - (valor * 0.10)
    elif tipo_combustivel == "g":
        valor = qtde_litros * gasolina
        if tipo_pagamento == "v" or tipo_pagamento == "d":
            valor = valor - (valor * 0.10)
    else:
        valor = qtde_litros * diesel
        if tipo_pagamento == "v" or tipo_pagamento == "d":
            valor = valor - (valor * 0.10)
...
def comprar_frutas(morango=0, uva=0):
    ...
    if morango <= 5:
        valor_a_pagar_morango = morango * 2.5
        if uva <= 5:
            valor_a_pagar_uva = uva * 1.8
        else:
            valor_a_pagar_uva = uva * 1.5
        valor_a_pagar = round((valor_a_pagar_morango + valor_a_pagar_uva), 2)
        if valor_a_pagar > 25 or (morango + uva) > 8:
            valor_a_pagar = valor_a_pagar - (valor_a_pagar * 0.10)
    else:
        valor_a_pagar_morango = morango * 2.2
        if uva <= 5:
            valor_a_pagar_uva = uva * 1.8
        else:
            valor_a_pagar_uva = uva * 1.5
