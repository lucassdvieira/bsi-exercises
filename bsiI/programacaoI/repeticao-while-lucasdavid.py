...
def quantidade_de_impares(valor_inicial, valor_final):
    ...
    """
    n, impar = valor_inicial, 0
    while n < valor_final-1:
        n += 1
        if n % 2 != 0:
            impar += 1
    return impar
...
def soma_dos_inteiros(valor1, valor2):
    ...
    """
    lista = []
    primeiro = valor1 if valor1 < valor2 else valor2
    segundo = valor2 if valor1 < valor2 else valor1
...
def potencia(base, expoente):
    ...
    n, potencia = 0, base
    while n < expoente-1:
        n, potencia = n + 1, potencia * base 
    return potencia if base > 1 and expoente != 0 else 1
...
def crescimento_populacional(populacao1, populacao2, crescimento1, crescimento2):
    ...
    while populacao1 < populacao2:
        anos += 0 if populacao1 < populacao2 and crescimento1 < crescimento2 else 1
        populacao1, populacao2 = populacao1 + (populacao1 * crescimento1) / 100, populacao2 + (populacao2 * crescimento2) / 100
    return anos
...
def é_primo(valor):
    ...
    c = 1
    while c < valor:
        if c >= 2 and valor % c == 0:
            return False
        c += 1
    return False if valor <= 1 else True
...
def serie2(n):
    ...
    count, valor, soma = 1, 1, 1
    while count < n:
        count, valor = count + 1, valor + 2 
        soma += count/valor
    return round(soma,2)
...
def serie_pi(n):
    ...
    """
    count, valor, soma, sinal = 1, 3, 4, -1
    while count < n:
        count, soma = count + 1, soma + (4/valor) * sinal
        valor, sinal = valor + 2, sinal * -1 
    return round(soma,6)
